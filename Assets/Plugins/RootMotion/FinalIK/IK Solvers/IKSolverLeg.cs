...
namespace RootMotion.FinalIK
{
    ...
    public class IKSolverLeg : IKSolver
    {
        ...
        {
            return new IKSolver.Point[5] { pelvis, thigh, calf, foot, toe };
        }

        public override IKSolver.Point GetPoint(Transform transform)
        {
            if (pelvis.transform == transform) return pelvis;
            if (thigh.transform == transform) return thigh;
            if (calf.transform == transform) return calf;
            if (foot.transform == transform) return foot;
            if (toe.transform == transform) return toe;
            return null;
        }
