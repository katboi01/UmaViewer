...
namespace RootMotion.FinalIK
{
    ...
    public class IKSolverArm : IKSolver
    {
        ...
        {
            return new IKSolver.Point[5] { chest, shoulder, upperArm, forearm, hand };
        }

        public override IKSolver.Point GetPoint(Transform transform)
        {
            if (chest.transform == transform) return chest;
            if (shoulder.transform == transform) return shoulder;
            if (upperArm.transform == transform) return upperArm;
            if (forearm.transform == transform) return forearm;
            if (hand.transform == transform) return hand;
            return null;
        }
