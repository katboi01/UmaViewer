...
namespace RootMotion.FinalIK
{
    ...
    public class IKSolverLookAt : IKSolver
    {
        ...
        public override IKSolver.Point[] GetPoints()
        {
            ...
            for (int i = 0; i < spine.Length; i++) allPoints[i] = spine[i];

            int eye = 0;
            for (int i = spine.Length; i < spine.Length + eyes.Length; i++)
            {
                allPoints[i] = eyes[eye];
                eye++;
            }

            if (head.transform != null) allPoints[allPoints.Length - 1] = head;
            return allPoints;
        }

        public override IKSolver.Point GetPoint(Transform transform)
        {
            foreach (IKSolverLookAt.LookAtBone b in spine) if (b.transform == transform) return b;
            foreach (IKSolverLookAt.LookAtBone b in eyes) if (b.transform == transform) return b;
            if (head.transform == transform) return head;
            return null;
        }
