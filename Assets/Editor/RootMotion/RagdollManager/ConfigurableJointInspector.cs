...
namespace RootMotion.Dynamics
{
    ...
    public class ConfigurableJointInspector : JointInspector
    {
        ...
        public static void DrawJoint(ConfigurableJoint joint, bool modifiable = true, float alphaMlp = 1f)
        {
            ...
            {
                float newLowAngularXLimit = DrawJointLimit(joint, lowX, axis, joint.lowAngularXLimit.limit, MlpAlpha(lowAngularXColor, alphaMlp), -25, modifiable);
                ...
                if (newLowAngularXLimit != 0 || (newLowAngularXLimit == 0 && newHighAngularXLimit != 0)) newHighAngularXLimit = DrawJointLimit(joint, highX, axis, joint.highAngularXLimit.limit, MlpAlpha(highAngularXColor, alphaMlp), 25, modifiable);
            ...
            {
                float newAngularYLimit = DrawJointLimit(joint, y, secondaryAxis, -joint.angularYLimit.limit, MlpAlpha(angularYColor, alphaMlp), 25, false);
                newAngularYLimit = DrawJointLimit(joint, y, secondaryAxis, -newAngularYLimit, MlpAlpha(angularYColor, alphaMlp), 25, modifiable);
            ...
            {
                float newAngularZLimit = DrawJointLimit(joint, z, crossAxis, -joint.angularZLimit.limit, MlpAlpha(angularZColor, alphaMlp), 25, false);
                newAngularZLimit = DrawJointLimit(joint, z, crossAxis, -newAngularZLimit, MlpAlpha(angularZColor, alphaMlp), 25, modifiable);
